...
function extractTrack(data: RecentTracksResponse["recenttracks"]): LastFmTrack | null {
  if (!data?.track) return null;

  const track = Array.isArray(data.track) ? data.track[0] : data.track;
  if (!track) return null;

  const artist = typeof track.artist === "string" ? track.artist : track.artist?.["#text"] ?? "Unknown";
...
export async function fetchLastFmSummary(username: string, apiKey: string): Promise<LastFmSummary> {
  if (!username || !apiKey) {
  ...
  const [recent, userInfo] = await Promise.all([
    fetchJson<RecentTracksResponse>(`${API_BASE}?${params.toString()}&method=user.getrecenttracks&limit=1`),
    fetchJson<UserInfoResponse>(`${API_BASE}?${params.toString()}&method=user.getinfo`),
  ]);
